...
def compute_color_histograms(cloud, using_hsv=False):
    ...
    hist_color_features = np.concatenate((channel_1_hist[0], channel_2_hist[0], channel_3_hist[0])).astype(np.float64)

    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
    normed_features = hist_color_features / np.sum(hist_color_features) 
...
def compute_normal_histograms(normal_cloud):
    ...
    # TODO: Concatenate and normalize the histograms
    hist_normal_features = np.concatenate((norm_x_hist[0], norm_y_hist[0], norm_z_hist[0])).astype(np.float64)

    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
    normed_features = hist_normal_features / np.sum(hist_normal_features)
